...
impl GitHubClient {
    ...
    fn auth_headers(&self) -> Result<Vec<(String, String)>, BindingError> {
        let token =
            self.config.token.as_ref().ok_or_else(|| {
                BindingError::CallError("GitHub token not configured".to_string())
            })?;
        Ok(vec![(
    ...
    fn get_definitions(&self) -> Result<Vec<ToolDefinition>, BindingError> {
        Ok(vec![
            ToolDefinition {
                ...
                input_schema: RepoInput::schema().to_string(),
            },
            ToolDefinition {
            ...
            ToolDefinition {
                name: "github_get_user".to_string(),
                description: "Get user information.".to_string(),
                input_schema: UserInput::schema().to_string(),
            },
        ])
    }
...
impl RepoInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"owner": {"type": "string"}, "repo": {"type": "string"}}, "required": ["owner", "repo"]})
    }
}
...
impl IssueInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"owner": {"type": "string"}, "repo": {"type": "string"}, "title": {"type": "string"}, "body": {"type": "string"}}, "required": ["owner", "repo", "title"]})
    }
}
