...
fn default_base_url() -> String { "https://api.twilio.com/2010-04-01".to_string() }

impl Default for TwilioConfig {
    fn default() -> Self { Self { account_sid: None, auth_token: None, from_number: String::new(), base_url: default_base_url() } }
}
...
impl TwilioClient {
    ...
    fn auth_header(&self) -> Result<String, BindingError> {
        let sid = self.config.account_sid.as_ref()
            .ok_or_else(|| BindingError::CallError("Twilio account_sid not configured".to_string()))?;
        let token = self.config.auth_token.as_ref()
            .ok_or_else(|| BindingError::CallError("Twilio auth_token not configured".to_string()))?;
        let creds = format!("{}:{}", sid, token);
        use base64::Engine;
        Ok(format!("Basic {}", base64::engine::general_purpose::STANDARD.encode(creds.as_bytes())))
    }

    fn request(&self, method: reqwest::Method, path: &str, body: Option<serde_json::Value>) -> Result<serde_json::Value, BindingError> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(BindingError::CallError(format!("Twilio API error: {}", text)));
        }
    ...
    pub fn send_sms(&self, to: &str, body: &str) -> Result<serde_json::Value, BindingError> {
        ...
        self.request(reqwest::Method::POST, &format!("/Accounts/{}/Messages.json", self.config.account_sid.as_ref().unwrap_or(&String::new())), Some(params))
    }
    ...
    pub fn make_call(&self, to: &str, url: &str) -> Result<serde_json::Value, BindingError> {
        ...
        self.request(reqwest::Method::POST, &format!("/Accounts/{}/Calls.json", self.config.account_sid.as_ref().unwrap_or(&String::new())), Some(params))
    }
}

/// Twilio tool plugin
#[derive(Debug, Clone)]
pub struct TwilioTool { client: Option<TwilioClient> }
...
impl Default for TwilioTool { fn default() -> Self { Self::new() } }

impl ToolPluginInstance for TwilioTool {
    fn get_definitions(&self) -> Result<Vec<ToolDefinition>, BindingError> {
        Ok(vec![
            ToolDefinition { name: "twilio_sms".to_string(), description: "Send an SMS message.".to_string(), input_schema: SmsInput::schema().to_string() },
            ToolDefinition { name: "twilio_call".to_string(), description: "Make a voice call with TwiML URL.".to_string(), input_schema: CallInput::schema().to_string() },
        ])
    }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SmsInput { pub to: String, pub body: String }
impl SmsInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"to": {"type": "string"}, "body": {"type": "string"}}, "required": ["to", "body"]}) } }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CallInput { pub to: String, pub twiml_url: String }
impl CallInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"to": {"type": "string"}, "twiml_url": {"type": "string"}}, "required": ["to", "twiml_url"]}) } }
