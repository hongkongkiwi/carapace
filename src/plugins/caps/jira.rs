...
impl ToolPluginInstance for JiraTool {
    fn get_definitions(&self) -> Result<Vec<ToolDefinition>, BindingError> {
        Ok(vec![
            ...
            ToolDefinition {
                name: "jira_transition".to_string(),
                description: "Transition an issue to a new state.".to_string(),
                input_schema: TransitionInput::schema().to_string(),
            },
            ToolDefinition {
                name: "jira_comment".to_string(),
                description: "Add a comment to an issue.".to_string(),
                input_schema: CommentInput::schema().to_string(),
            },
        ])
    }
    ...
    fn invoke(
        ...
        match name {
            "jira_create_issue" => {
                ...
                let result =
                    client.create_issue(&input.summary, &input.description, &input.issue_type)?;
                Ok(ToolResult {
                    success: true,
...
impl IssueInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"summary": {"type": "string"}, "description": {"type": "string"}, "issue_type": {"type": "string"}}, "required": ["summary", "description", "issue_type"]})
    }
}
...
impl GetIssueInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"issue_key": {"type": "string"}}, "required": ["issue_key"]})
    }
}
...
impl TransitionInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"issue_key": {"type": "string"}, "transition_id": {"type": "string"}}, "required": ["issue_key", "transition_id"]})
    }
}
...
impl CommentInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"issue_key": {"type": "string"}, "comment": {"type": "string"}}, "required": ["issue_key", "comment"]})
    }
}
