...
impl SendGridClient {
    ...
    fn auth_headers(&self) -> Result<Vec<(String, String)>, BindingError> {
        let key = self.config.api_key.as_ref().ok_or_else(|| {
            BindingError::CallError("SendGrid API key not configured".to_string())
        })?;
        Ok(vec![(
    ...
    /// Send template email
    pub fn send_template_email(
        &self,
        to: &str,
        template_id: &str,
        dynamic_data: serde_json::Value,
    ) -> Result<serde_json::Value, BindingError> {
        let email_body = json!({
...
/// SendGrid tool plugin
#[derive(Debug, Clone)]
pub struct SendGridTool {
    client: Option<SendGridClient>,
}
...
impl ToolPluginInstance for SendGridTool {
    fn get_definitions(&self) -> Result<Vec<ToolDefinition>, BindingError> {
        Ok(vec![
            ToolDefinition {
                name: "sendgrid_send".to_string(),
                description: "Send an email.".to_string(),
                input_schema: SendInput::schema().to_string(),
            },
            ToolDefinition {
                name: "sendgrid_template".to_string(),
                description: "Send a template email.".to_string(),
                input_schema: TemplateInput::schema().to_string(),
            },
        ])
    }
    ...
    fn invoke(
        ...
        match name {
            ...
            "sendgrid_template" => {
                ...
                let result = client.send_template_email(
                    &input.to,
                    &input.template_id,
                    input.dynamic_data.unwrap_or_else(|| json!({})),
                )?;
                Ok(ToolResult {
                    success: true,
                    result: Some(serde_json::to_string(&result)?),
...
impl SendInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"to": {"type": "string"}, "subject": {"type": "string"}, "body": {"type": "string"}, "html": {"type": "string"}}, "required": ["to", "subject", "body"]})
    }
}
...
impl TemplateInput {
    fn schema() -> serde_json::Value {
        json!({"type": "object", "properties": {"to": {"type": "string"}, "template_id": {"type": "string"}, "dynamic_data": {"type": "object"}}, "required": ["to", "template_id"]})
    }
}
