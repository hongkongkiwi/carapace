...
fn default_base_url() -> String { "https://api.notion.com/v1".to_string() }
...
impl NotionClient {
    ...
    fn auth_headers(&self) -> Result<Vec<(String, String)>, BindingError> {
        let key = self.config.api_key.as_ref()
            .ok_or_else(|| BindingError::CallError("Notion API key not configured".to_string()))?;
        Ok(vec![("Authorization".to_string(), format!("Bearer {}", key)), ("Notion-Version".to_string(), "2022-06-28".to_string())])
    }
    ...
    fn request(&self, method: reqwest::Method, endpoint: &str, body: Option<serde_json::Value>) -> Result<serde_json::Value, BindingError> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(BindingError::CallError(format!("Notion API error: {}", text)));
        }
        resp.json().map_err(|e| BindingError::CallError(format!("Parse error: {}", e)))
    }

    /// Create page
    pub fn create_page(&self, parent_id: &str, title: &str, content: &str) -> Result<serde_json::Value, BindingError> {
        let body = json!({
    ...
    pub fn query_database(&self, database_id: &str) -> Result<serde_json::Value, BindingError> {
        self.request(reqwest::Method::POST, &format!("/databases/{}/query", database_id), None)
    }

    /// Update page
    pub fn update_page(&self, page_id: &str, properties: serde_json::Value) -> Result<serde_json::Value, BindingError> {
        self.request(reqwest::Method::PATCH, &format!("/pages/{}", page_id), Some(properties))
    }
}

/// Notion tool plugin
#[derive(Debug, Clone)]
pub struct NotionTool { client: Option<NotionClient> }
...
impl Default for NotionTool { fn default() -> Self { Self::new() } }

impl ToolPluginInstance for NotionTool {
    fn get_definitions(&self) -> Result<Vec<ToolDefinition>, BindingError> {
        Ok(vec![
            ToolDefinition { name: "notion_create_page".to_string(), description: "Create a new page.".to_string(), input_schema: PageInput::schema().to_string() },
            ToolDefinition { name: "notion_get_page".to_string(), description: "Get page details.".to_string(), input_schema: GetPageInput::schema().to_string() },
            ToolDefinition { name: "notion_query_db".to_string(), description: "Query a database.".to_string(), input_schema: QueryInput::schema().to_string() },
        ])
    }
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PageInput { pub parent_id: String, pub title: String, pub content: String }
impl PageInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"parent_id": {"type": "string"}, "title": {"type": "string"}, "content": {"type": "string"}}, "required": ["parent_id", "title", "content"]}) } }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GetPageInput { pub page_id: String }
impl GetPageInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"page_id": {"type": "string"}}, "required": ["page_id"]}) } }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QueryInput { pub database_id: String }
impl QueryInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"database_id": {"type": "string"}}, "required": ["database_id"]}) } }
