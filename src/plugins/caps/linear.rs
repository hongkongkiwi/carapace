...
fn default_base_url() -> String { "https://api.linear.app/graphql".to_string() }

impl Default for LinearConfig {
    fn default() -> Self { Self { api_key: None, base_url: default_base_url() } }
}
...
impl LinearClient {
    ...
    fn request(&self, query: &str, variables: Option<serde_json::Value>) -> Result<serde_json::Value, BindingError> {
        let key = self.config.api_key.as_ref()
            .ok_or_else(|| BindingError::CallError("Linear API key not configured".to_string()))?;
        let body = json!({ "query": query, "variables": variables });
        let resp = self.http_client.post(&self.config.base_url)
        ...
        if !resp.status().is_success() {
            ...
            return Err(BindingError::CallError(format!("Linear API error: {}", text)));
        }
        let result: serde_json::Value = resp.json().map_err(|e| BindingError::CallError(format!("Parse error: {}", e)))?;
        if let Some(errors) = result.get("errors") {
            return Err(BindingError::CallError(format!("Linear GraphQL errors: {}", errors)));
        }
        Ok(result)
    }

    /// Create issue
    pub fn create_issue(&self, team_id: &str, title: &str, description: Option<&str>) -> Result<serde_json::Value, BindingError> {
        ...
        let variables = json!({ "teamId": team_id, "title": title, "description": description.unwrap_or("") });
    ...
    /// List issues
    pub fn list_issues(&self, team_id: &str, limit: i32) -> Result<serde_json::Value, BindingError> {
...
/// Linear tool plugin
#[derive(Debug, Clone)]
pub struct LinearTool { client: Option<LinearClient> }
...
impl Default for LinearTool { fn default() -> Self { Self::new() } }

impl ToolPluginInstance for LinearTool {
    fn get_definitions(&self) -> Result<Vec<ToolDefinition>, BindingError> {
        Ok(vec![
            ToolDefinition { name: "linear_create_issue".to_string(), description: "Create a new issue.".to_string(), input_schema: IssueInput::schema().to_string() },
            ToolDefinition { name: "linear_get_issue".to_string(), description: "Get issue details.".to_string(), input_schema: GetIssueInput::schema().to_string() },
            ToolDefinition { name: "linear_list_issues".to_string(), description: "List issues in a team.".to_string(), input_schema: ListInput::schema().to_string() },
        ])
    }
    ...
    fn invoke(&self, name: &str, params: &str, _ctx: ToolContext) -> Result<ToolResult, BindingError> {
        ...
        match name {
            "linear_create_issue" => {
                ...
                let result = client.create_issue(&input.team_id, &input.title, input.description.as_deref())?;
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IssueInput { pub team_id: String, pub title: String, pub description: Option<String> }
impl IssueInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"team_id": {"type": "string"}, "title": {"type": "string"}, "description": {"type": "string"}}, "required": ["team_id", "title"]}) } }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GetIssueInput { pub issue_id: String }
impl GetIssueInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"issue_id": {"type": "string"}}, "required": ["issue_id"]}) } }

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ListInput { pub team_id: String, pub limit: Option<i32> }
impl ListInput { fn schema() -> serde_json::Value { json!({"type": "object", "properties": {"team_id": {"type": "string"}, "limit": {"type": "integer"}}, "required": ["team_id"]}) } }
